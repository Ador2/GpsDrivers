	...
	, _satellite_info(satellite_info)
{
	...
	// initialize the InertialSense message parsing
...
int GPSDriverInertialSense::configure(unsigned &baud, OutputMode output_mode)
{
  ...
  messageSize = is_comm_get_data(DID_GPS1_NAV, 0, 0, 100);
  write(message_buffer_, messageSize);
...
void GPSDriverInertialSense::GPS_callback(gps_nav_t *data)
{
  _got_sat_info = true;
  _got_gps = true;
  ...
  _gps_position->s_variance_m_s = data->sAcc;

  _gps_position->eph = data->hAcc;
  _gps_position->epv = data->vAcc;
  ...
  _gps_position->vel_n_m_s = (int32_t)(data->velEcef[0]);
  _gps_position->vel_e_m_s = (int32_t)(data->velEcef[1]);
  _gps_position->vel_d_m_s = (int32_t)(data->velEcef[2]);
  ...
  uint32_t fix_type = data->status & GPS_STATUS_FIX_FLAGS_MASK;
  switch(fix_type)
  {
  case (GPS_STATUS_FIX_STATUS_NO_FIX):
    _gps_position->fix_type = 0;
    break;
  case (GPS_STATUS_FIX_STATUS_2D_FIX):
    _gps_position->fix_type = 2;
    break;
  case (GPS_STATUS_FIX_STATUS_3D_FIX):
...
int GPSDriverInertialSense::receive(unsigned timeout)
{
  ...
  while ((gps_absolute_time() < time_started + timeout*1000) && !(_got_gps || _got_sat_info))
  {
    ...
    {
      uint32_t message_type = is_comm_parse(buf[i]);
      switch(message_type)
      {
      case DID_NULL:
        // no message yet
